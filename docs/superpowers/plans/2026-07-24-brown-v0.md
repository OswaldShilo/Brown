...
- Produces: `RenderableAnnotation { quote: string; note: string; range: Range; side: Side; rect: Rect }`, `RenderableDiagram { title: string; labels: string[]; side: Side; rect: Rect }`, and `annotateResponse(container: Element, zones: { left: Rect; right: Rect }, apiKey: string): Promise<{ notes: RenderableAnnotation[]; diagram?: RenderableDiagram }>`. The diagram (when present) is placed using the same `Margins` allocator as the notes, in whichever side has more remaining room, after all notes are placed — it has no text anchor, so its `rect.y0` starts at that side's current cursor. Task 15 (`main.tsx`) consumes `annotateResponse` directly; Task 9/10 (`strokes.ts`/`diagram.ts`) consume the `RenderableAnnotation`/`RenderableDiagram` shapes indirectly via Task 15's rendering glue.
...
7. Check the box, click the pet again — confirm the pet switches to its busy animation, then after the Gemini call resolves, confirm at least one underline + handwritten-style note appears in the page's left or right margin, anchored near a bold/heading phrase in the response. If the response describes a multi-step process, also confirm a sketchy node-and-connector diagram appears below the notes in one margin.
...
- **Type consistency:** `Rect`/`Side` (Task 2) are reused as-is by Task 8 and Task 15; `Candidate` (Task 4) flows unchanged through Task 6, 7, and 8; `GeminiAnnotationResult`/`NoteAnnotation`/`DiagramSpec` (Task 6) flow unchanged through Task 7 and 8; `RenderableAnnotation`/`RenderableDiagram` (Task 8) are consumed as-is by Task 15's `drawAnnotation`/`drawDiagram`.
- **Fixed during pre-flight review (before any task was dispatched):** the original draft had `annotateResponse` (Task 8) return only `RenderableAnnotation[]`, silently dropping `result.diagram` from the Gemini response even though the Global Constraints require "at most 1 diagram per response" to actually render. Task 8 now also places the diagram via the same `Margins` allocator and returns `{ notes, diagram? }`; Task 15's `runPipelineFor`/`drawDiagram` were updated to consume it. Task 8's and Task 15's test/interface blocks above already reflect the fix — no further action needed before dispatching Task 1.
